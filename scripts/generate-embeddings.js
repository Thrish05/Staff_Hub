...
const MODEL = process.env.EMBED_MODEL || "all-minilm";
...
//     if (formattedVector)
//       await client.query(update, [formattedVector, row.scopus_research_id]);
//   }
...
//   const { rows } = await client.query(select);
//   for (const row of rows) {
//     const text = `${row.name_of_faculty} of id ${row.faculty_id} did a project titled "${row.project_title}" funded by ${row.funding_agency} in ${row.year_of_award}`;
//     const embedding = await getEmbedding(text);
//     const formattedVector = `[${embedding.join(',')}]`;
//     if (formattedVector)
//       await client.query(update, [formattedVector, row.project_title, row.name_of_faculty]);
//   }
